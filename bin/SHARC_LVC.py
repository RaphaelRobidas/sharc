...
print("Import: CPU time: % .3f s, wall time: %.3f s"%(time.process_time() - tc, time.time() - tt))
  ...
  for imult in range(nmult):
    ...
    if not dim == 0:
      ...
      for ms in range(imult+1):
        ...
        offs += dim
#  print("QMout1: CPU time: % .3f s, wall time: %.3f s"%(time.process_time() - tc, time.time() - tt))
...
#  print("QMout2: CPU time: % .3f s, wall time: %.3f s"%(time.process_time() - tc, time.time() - tt))
...
#  print("QMout3: CPU time: % .3f s, wall time: %.3f s"%(time.process_time() - tc, time.time() - tt))
...
def main():
  ...
  print("SH2LVC: CPU time: % .3f s, wall time: %.3f s"%(time.process_time() - tc, time.time() - tt))

  QMout=getQMout(QMin,SH2LVC)
  print("QMout:  CPU time: % .3f s, wall time: %.3f s"%(time.process_time() - tc, time.time() - tt))

  # This print(routine takes about 10 ms, i.e. ~5% of the total execution time)
  printQMout(QMin,QMout)
  #print("Print:  CPU time: % .3f s, wall time: %.3f s"%(time.process_time() - tc, time.time() - tt))

  # Write QMout
  writeQMout(QMin,QMout,'QM.in')
  print("Write:  CPU time: % .3f s, wall time: %.3f s"%(time.process_time() - tc, time.time() - tt))

  print("Final:  CPU time: % .3f s, wall time: %.3f s"%(time.process_time() - tc, time.time() - tt))
  print('#================ END ================#')
